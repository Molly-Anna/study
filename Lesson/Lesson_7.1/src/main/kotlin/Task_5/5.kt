 ...
 * @param Regex("""([a-zA-Z-0-9-_]+)@([a-z]+).([a-z]{1,4})\b""")
 * логин - ([a-zA-Z-0-9-_]+)
 * собака - @
 * домен - ([a-z]+)
 * точка - .
 ...
 *
 * @param result = regex.findAll(email).map { it.groupValues[1] } - возвращает все нахождения
 * и преобразует вывод без квадратных ковычек и через ; находит последовательно все совпадения из списка
 * и выводит только первую группу захваенного списка(первые скобки).
 *
 * @return result.toList().joinToString("; ") - возвращает все нахождения
 * и преобразует вывод без квадратных ковычек и через ;
 */
