 ...
 *
 * @sample matchEntire(number) - проверяет полсностью ли строка соответствует регулярному выражению.
 * @return matchResult?.groupValues?.get(2) ?: "" - если matchResult не null и есть полное соотвествие,
 * то возвращается указанный номер, если соответсвия нет, то возвращается null и пустая строка
 */
